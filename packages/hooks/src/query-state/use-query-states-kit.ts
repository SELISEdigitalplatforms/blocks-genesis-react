...
type QueryKey<TSchema extends QueryStatesKitSchema> = Extract<keyof Values<TSchema>, string>;
...
export function useQueryStatesKit<TSchema extends QueryStatesKitSchema>(
  ...
  const resolveOptions = (override?: QueryUpdateOptions): QueryUpdateOptions => ({
    history: resolvedBehavior.history,
    shallow: resolvedBehavior.shallow,
    scroll: resolvedBehavior.scroll,
    ...override,
  ...
  const setQuery: UseQueryStatesKitReturn<TSchema>["setQuery"] = (patch, updateOptions) => {
    if (typeof patch === "function") {
      return setRawQuery((previous) => patch(previous), resolveOptions(updateOptions));
    }

    return setRawQuery(patch, resolveOptions(updateOptions));
  };

  const resetQuery: UseQueryStatesKitReturn<TSchema>["resetQuery"] = (keys, updateOptions) => {
    const targetKeys =
      keys ?? (Object.keys(schema) as Array<QueryKey<TSchema>>);

    const resetPatch: Record<string, unknown> = {};
    for (const key of targetKeys) {
      resetPatch[key] = key in defaults ? defaults[key as keyof Values<TSchema>] : null;
    }

    return setRawQuery(resetPatch as QueryPatch<TSchema>, resolveOptions(updateOptions));
  };

  const clearQuery: UseQueryStatesKitReturn<TSchema>["clearQuery"] = (keys, updateOptions) => {
    if (!keys) {
    ...
    return setRawQuery(clearPatch as QueryPatch<TSchema>, resolveOptions(updateOptions));
  };
  ...
  const resetPage: UseQueryStatesKitReturn<TSchema>["resetPage"] = (pageKey, updateOptions) => {
    const key = (pageKey ?? resolvedBehavior.pageKey) as QueryKey<TSchema>;
    const nextValue = key in defaults ? defaults[key as keyof Values<TSchema>] : null;
  ...
  const updateAndResetPage: UseQueryStatesKitReturn<TSchema>["updateAndResetPage"] = (
    ...
    const key = (pageKey ?? resolvedBehavior.pageKey) as QueryKey<TSchema>;
    const nextPage = key in defaults ? defaults[key as keyof Values<TSchema>] : null;

    return setRawQuery((previous) => {
      const partialPatch = typeof patch === "function" ? patch(previous) : patch;
      if (!partialPatch) {
        return {
          [key]: nextPage,
        } as QueryPatch<TSchema>;
