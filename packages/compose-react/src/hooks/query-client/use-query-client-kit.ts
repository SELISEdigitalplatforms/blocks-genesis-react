...
export function useQueryClientKit(): UseQueryClientKitReturn {
  ...
  return {
    ...
    cancel: (key, filters, options) =>
      client.cancelQueries(withQueryKey(key, filters), options),
    refetch: (key, filters, options) =>
      client.refetchQueries(withQueryKey(key, filters), options),
    remove: (key, filters) => client.removeQueries(withQueryKey(key, filters)),
    reset: (key, filters, options) =>
      client.resetQueries(withQueryKey(key, filters), options),
    getData: <TData = unknown>(key: BlocksQueryKey) =>
      client.getQueryData<TData>(key),
    setData: <TData = unknown>(
      key: BlocksQueryKey,
      updater: Updater<TData | undefined, TData | undefined>,
      options?: SetDataOptions,
    ): TData | undefined =>
      client.setQueryData<TData>(key, updater, options) as TData | undefined,
    getQueriesData: (key, filters) =>
      client.getQueriesData(withQueryKey(key, filters)),
